...
Formula::Formula(const Parser& tp, const string& latex, bool preprocess, bool isMathMode)
  ...
  // xournal-qt: an empty argument ({}, or none at the end of the source) is an empty atom, not a null one that
  // the atoms made from it dereference (found by fuzzing: \raisebox, \reflectbox, \shoveleft, \mathbin, ...)
  if (_root == nullptr) _root = sptrOf<EmptyAtom>();
}
