...
- [ ] A library with a plain PDF, a PDF with notes (a PDF annotated and saved in PDF files mode) and a PDF text
      document (New text document… in PDF files mode): the badges read "PDF" (red), "PDF ✎" (red) and "PDF MD"
...
- [ ] An archive PDF (⋮ → Export for the archive…) reads "PDF/A ✎"; one of a text document "PDF/A MD" (blue).
...
- [ ] Annotate a plain PDF and save: its card turns from "PDF" into "PDF ✎" within a second or two. "Open as PDF
      document" of a `.md`: the new card reads "PDF MD".
