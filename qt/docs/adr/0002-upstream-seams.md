...
| `src/core/view/MarkdownHook.h` (new), `src/core/view/LayerView.cpp` | Markdown boxes: the texts of a layer named "Markdown" go to a renderer the frontend registers (`xoj::view::markdownTextRenderer`, an atomic function pointer; xournal-qt: `qt/src/markdown/MdBox.cpp`). They are not culled by their bounding box, which is the source's, not that of what is drawn. Without a renderer (upstream) nothing changes: the source is shown as text. |
