...
| `src/core/model/XojPage.h/.cpp`, `control/xojfile/XmlAttrs.h`, `XmlParser.cpp`, `DocumentBuilderInterface.h`, `LoadHandler.h/.cpp`, `SaveHandler.cpp` | Bookmarks on pages ([bookmarks.md](../bookmarks.md)). A page holds an optional bookmark label (`getBookmark` / `setBookmark`; copied with the page), saved as the page attribute `xqt-bookmark="label"` only when the page is bookmarked (an empty label: the app names it "Page N"); the parser passes it to the builder through a new virtual with an empty default, like `notespace`. Upstream's parser looks attributes up by name and ignores the others, so Xournal++ opens such files without a message; saving there drops the bookmarks. |
| `src/core/view/background/RuledBackgroundView.h/.cpp`, `LinedBackgroundView.cpp` | Ruled and lined paper on pages smaller than A5 (flashcards). A frontend may draw the ruling to scale (`xoj::view::ruledScale`, an atomic function pointer of the page size): the space above and below the lines and the default margin line (1 inch; not one set in the page type) are multiplied by it; the first line moves up by whole lines, so the lines stay on upstream's grid (snapping to them still fits). xournal-qt: `qt/src/session/PageMargins.cpp` (the text margins' scale: 1 on A5 and bigger, 0.5 on A7), set by `AppContext`, so the canvas, thumbnails and every export draw it alike. Without one (upstream, and the CLI of the golden tests) nothing changes. |
