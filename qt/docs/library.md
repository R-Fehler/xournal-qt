...
- A document never saved: in the cache (`~/.cache/xournal-qt/pasted-pages`) until its first save puts it next to
  the `.xopp` by the rules above. Closed without saving, it is removed.
